...
export const SKILLS={
  ...
  threeHeadedDingo:{name:'Three-Headed Dingo',round:3,icon:'🐕',description:'May initiate up to three predations in the same predation stage.'},
  ...
  taipanVenom:{name:'Inland Taipan Venom',round:3,icon:'🐍',description:'When eliminated by a predator, eliminate the killer as well; both lose all predation cards.'},
...
export function makePlayer(index,name,roleId){
  return{id:index+1,seat:index+1,name:name||`Player ${index+1}`,roleId,life:20,alive:true,skills:[],predationCards:Array.from({length:10},(_,i)=>i+1),hostId:null,packmates:[],torpor:false,stageUses:{evasiveLeap:false,decoy:false,attackCount:0},skillUses:{},protected:false,lastPredatorId:null,tradedLifeReceived:0,decoyTargets:[]};
}
...
export function resetPredationStage(state){state.players.forEach(p=>{p.torpor=false;p.stageUses={evasiveLeap:false,decoy:false,attackCount:0};});}
...
export function resolvePredation(state,action){
  ...
  const attacker=getAlivePlayer(state,action.attackerId);let defender=getAlivePlayer(state,action.defenderId);
  if(attacker.id===defender.id)throw new Error('Cannot prey on yourself.');if(defender.torpor)throw new Error('Target is in torpor and cannot be targeted this stage.');if(defender.protected)throw new Error('Target is in the Nature Reserve for this round.');
  ...
  const maxAttacks=attacker.skills.includes('threeHeadedDingo')?3:1;if(attacker.stageUses.attackCount>=maxAttacks)throw new Error(`This player may only initiate ${maxAttacks} predation(s) this stage.`);
  if(action.decoyId){const decoy=getAlivePlayer(state,action.decoyId);if(!decoy.skills.includes('decoy'))throw new Error('Selected player does not own Decoy.');if(decoy.stageUses.decoy)throw new Error('Decoy has already been used this stage.');if(decoy.id===attacker.id)throw new Error('Attacker cannot intercept their own attack.');if(decoy.decoyTargets.includes(defender.id))throw new Error('Decoy cannot intercept the same original target twice.');decoy.stageUses.decoy=true;decoy.decoyTargets.push(defender.id);defender=decoy;}
  if(action.useEvasiveLeap){if(!defender.skills.includes('evasiveLeap'))throw new Error('Defender does not own Evasive Leap.');if(defender.stageUses.evasiveLeap)throw new Error('Evasive Leap has already been used this stage.');if(action.decoyId)throw new Error('A Decoy interception cannot be redirected by Evasive Leap.');const nextId=nextLivingSeat(state,defender.id);defender.stageUses.evasiveLeap=true;defender=getAlivePlayer(state,nextId);}
  if(!attacker.skills.includes('apexBloodline')){const cardIndex=attacker.predationCards.indexOf(defender.seat);if(cardIndex<0)throw new Error(`Predation card ${defender.seat} is not available.`);attacker.predationCards.splice(cardIndex,1);}
  attacker.stageUses.attackCount+=1;defender.lastPredatorId=attacker.id;
  if(defender.skills.includes('echidnaSpines'))loseLife(state,attacker,1,defender.id,'Echidna Spines');
  if(!attacker.alive)return{winnerId:defender.id,loserId:attacker.id,amount:0,redirectedDefenderId:defender.id,attackerEliminatedBySpines:true};
  ...
  return{winnerId:winner.id,loserId:loser.id,amount,attackerWins,redirectedDefenderId:defender.id,tailDropUsed:canTailDrop};
}
export function resolvePackFollowUps(state,leaderId,defenderId){
  ...
  for(const pupId of leader.packmates){const pup=state.players.find(p=>p.id===pupId);if(!pup||!pup.alive||!defender.alive)continue;const pupWins=compareRoles(pup,defender,pup.skills,defender.skills)>=0,amount=predationValue(state);defender.lastPredatorId=pup.id;if(pupWins){loseLife(state,defender,amount,pup.id,'Pack attack');const leaderShare=Math.min(3,amount);gainLife(state,leader,leaderShare,'Pack share');gainLife(state,pup,amount-leaderShare,'Pack attack');}else{pup.lastPredatorId=defender.id;loseLife(state,pup,amount,defender.id,'Pack attack');gainLife(state,defender,amount,'Pack defence');}results.push({pupId,pupWins,amount});}
  return results;
}
...
export function eliminatePlayer(state,playerId,killerId=null,reason=''){
  const player=state.players.find(p=>p.id===playerId);if(!player||!player.alive)return;const cards=[...player.predationCards],creditedPredatorId=killerId||player.lastPredatorId,venom=player.skills.includes('taipanVenom')&&creditedPredatorId;
  player.alive=false;player.life=0;player.predationCards=[];state.log.unshift(`${player.name} was eliminated${reason?` (${reason})`:''}.`);
  if(!venom&&creditedPredatorId){const predator=state.players.find(p=>p.id===creditedPredatorId);if(predator&&predator.alive){gainLife(state,predator,3,'Elimination bonus');predator.predationCards.push(...cards);predator.predationCards.sort((a,b)=>a-b);state.log.unshift(`${predator.name} gained 3 life and ${cards.length} predation card(s) from the elimination.`);}}
  state.players.filter(p=>p.alive&&p.skills.includes('scavenger')).forEach(s=>{s.life+=5;state.log.unshift(`${s.name} gained 5 life from Scavenger.`);});
  if(venom){const killer=state.players.find(p=>p.id===creditedPredatorId);if(killer&&killer.alive){killer.alive=false;killer.life=0;killer.predationCards=[];state.log.unshift(`${killer.name} was eliminated by Inland Taipan Venom.`);state.players.filter(p=>p.alive&&p.skills.includes('scavenger')).forEach(s=>{s.life+=5;state.log.unshift(`${s.name} gained 5 life from Scavenger.`);});}}
}
